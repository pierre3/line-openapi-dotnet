...
/// <summary>
/// Extensions that register <see cref="InsightClient"/> with the DI container.
///
...
/// hosts from <see cref="LineInsightOptions.AllowedHosts"/>.
...
public static class ServiceCollectionExtensions
{
    ...
    public const string HttpClientName = "Line.OpenApi.Insight";

    /// <summary>
    /// Registers <see cref="InsightClient"/> with a static (long-lived) channel access token.
    /// </summary>
    public static IServiceCollection AddLineInsight(
        this IServiceCollection services,
        Action<LineInsightOptions> configure)
    {
        if (services is null) throw new ArgumentNullException(nameof(services));
        if (configure is null) throw new ArgumentNullException(nameof(configure));

        services.AddOptions<LineInsightOptions>()
            .Configure(configure)
            .Validate(o => !string.IsNullOrWhiteSpace(o.ChannelAccessToken),
                "LineInsightOptions.ChannelAccessToken is required.");

        return services.AddLineInsight(sp =>
        {
            var opts = sp.GetRequiredService<IOptions<LineInsightOptions>>().Value;
    ...
    /// <summary>
    /// Registers <see cref="InsightClient"/> with an arbitrary authentication provider.
    /// Use this overload to inject a refreshing token provider (Line.OpenApi.ChannelAccessToken); it is
    /// the injection path that avoids a Line.OpenApi.Insight -> Line.OpenApi.ChannelAccessToken dependency.
    /// </summary>
    public static IServiceCollection AddLineInsight(
        this IServiceCollection services,
        Func<IServiceProvider, IAuthenticationProvider> authProviderFactory)
    {
        ...
        if (!services.Any(d => d.ServiceType == typeof(LineInsightMarker)))
        {
            services.AddSingleton<LineInsightMarker>();
        ...
        services.TryAddSingleton(sp =>
        {
            ...
            return new InsightClient(authProvider, httpClient);
        });
    ...
    private sealed class LineInsightMarker { }
}
