...
- **（実装完了・ゲート済・main マージ済み）CLI / MCP ツール `Line.OpenApi.Tools` の構築。** ローカル PC 用 `dotnet tool`（コマンド名 `line`）。同一ロジックを CLI（**Cocona 2.2.0**）と MCP サーバ（公式 **ModelContextProtocol 1.4.0 stable**）で両出し。共通サービス層を DI 共有。機能 A. トークン管理 / B. メッセージ送信・Bot 照会 / C. Webhook 開発支援 / D. LIFF 管理 を全実装。**仕様 = `docs/CLI-MCP-tool-spec.md`。**
  ...
  - **follow-up（非ブロッキング・残）:** Cocona パースエラーの exit 2 化（既定 1）／CancellationToken 配線（`listen` 以外は `None`）／Windows 明示 ACL（現状は継承 ACL＋正確な警告）／**CLI パリティ: `message schema` サブコマンド・`message ... --dry-run`（検証本体は共通サービス層にあり容易）**。
...
- **将来パッケージ・カバレッジ / ロードマップ:** `docs/coverage-roadmap.md`（2026-07-15 調査・一次情報裏取り）に集約。要点: line-openapi の spec 9 本中 4 本取り込み済み／**Rich Menu は messaging-api.yml に含まれ生成済み**（残ギャップは使い勝手＝ファサード＋画像 content-type ヘルパ）／未取り込み 5 本（insight・manage-audience・module・module-attach・shop）／spec 非存在の主要機能に **LINE Login v2.1 + OIDC ID Token 検証**（需要最大・手書き新パッケージ候補）・Social API 友だち関係。推奨優先順位も同ファイル参照。
