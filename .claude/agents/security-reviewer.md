...
3. **Webhook 署名検証（HMAC-SHA256）** — 署名比較が **定数時間比較**（`CryptographicOperations.FixedTimeEquals`）であること（`Line.Core/Webhook/WebhookSignatureValidator.cs`。TFM は net10.0 単一）。早期 return による長さリークや `==` 比較がないか。
